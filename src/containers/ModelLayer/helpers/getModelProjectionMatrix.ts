...
const MODEL_COORDINATES = MercatorCoordinate.fromLngLat(
  MODEL_POSITION,
  MODEL_ALTITUDE
);
...
const MODEL_ROTATION_MAT_X = new Matrix4().makeRotationAxis(
  new Vector3(1, 0, 0),
  MODEL_TRANSFORM.rotateX
);

const MODEL_ROTATION_MAT_Y = new Matrix4().makeRotationAxis(
  new Vector3(0, 1, 0),
  MODEL_TRANSFORM.rotateY
);

const MODEL_ROTATION_MAT_Z = new Matrix4().makeRotationAxis(
  new Vector3(0, 0, 1),
  MODEL_TRANSFORM.rotateZ
);
...
export function getModelProjectionMatrix(m: number[]) {
  ...
  const projectionMatrix = new Matrix4()
    .makeTranslation(
      MODEL_TRANSFORM.translateX,
      MODEL_TRANSFORM.translateY,
      MODEL_TRANSFORM.translateZ
    )
    .scale(scaleVector)
